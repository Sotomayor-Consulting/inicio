---
title: "What Stripe Needs to Approve Your Account: Complete Guide 2026"
description: "What Stripe needs to approve your account"
cardImage: "@/images/insights/que-necesita-stripe-para-aprobar-tu-cuenta.avif"
cardImageAlt: "Stripe approval screen with documents and green checkmark"
---

Getting your Stripe account approved is the first step to accepting online payments. However, many accounts are rejected or remain pending for days or weeks due to incorrect documentation, inconsistent information, or missing requirements.

In this guide, we explain **exactly what Stripe needs to approve your account in 2026**, with a detailed checklist, accepted documents, and tips to avoid rejections.

## 1. The Stripe Approval Process

### Approval Stages

### Types of Review

- **Timeline:** 24-72 hours -- 1-4 weeks
- **Documents:** ID + Proof of address -- + Company documents + BOI + Licenses
- **Verification:** Basic selfie -- Video selfie + additional documentation
- **Risk analysis:** Automatic (ML) -- Manual + automatic
> **Fact:** Most accounts are approved within 24-48 hours. Rejected accounts typically have documentation or information inconsistency issues.

## 2. Basic Requirements (All Accounts)

### Mandatory Personal Documents

- **Identity document:** Valid, with photo, not expired -- Passport, driver's license, national ID
- **Proof of address:** Less than 3 months old -- Utility bill, phone bill, internet bill, or bank statement
- **Date of birth:** Over 18 years old -- Verified with document
- **Phone number:** Valid (preferably in your name) -- Used for 2FA verification
- **Email:** Verified access -- Stripe sends confirmations and alerts
### Personal Information

- Full name (exactly as on document)
- Complete residential address
- Tax ID (SSN, EIN, CPF, RFC, etc.)
- Profession and industry
- Website or online presence

### Website or Online Presence

Stripe **always** verifies your website or application. Minimum requirements:

- **Live website:** ✅ Yes -- Cannot be "under construction"
- **Privacy policy:** ✅ Yes -- Explain how data is handled
- **Terms of service:** ✅ Yes -- Rules for using the service
- **Refund policy:** ✅ Yes -- Return timelines and conditions
- **Contact information:** ✅ Yes -- Email, phone, or form
- **Clear product description:** ✅ Yes -- What you sell or offer
- **SSL/HTTPS:** ✅ Yes -- Mandatory security
> **Tip:** If you don't have a website, use Stripe Payment Links. Stripe can approve accounts with just links, but verification is more rigorous.

## 3. Requirements for Business Accounts

### Company Documents

- **Certificate of incorporation:** Proves the company legally exists -- Companies Registry, Secretary of State
- **EIN (Employer Identification Number):** Business tax ID number -- IRS (free)
- **Business proof of address:** Utility bill or lease agreement -- Service provider, real estate agency
- **Articles of incorporation:** Document defining company structure -- Lawyer or accountant
- **Register of Directors:** List of directors (some countries) -- Company or regulatory agency
### Ultimate Beneficial Owner (UBO) Information

For business accounts, Stripe requires:

- **Full name:** Of each owner with >25% stake
- **Date of birth:** Of each owner
- **Address:** Residential address of each owner
- **Identity document:** Copy for each owner
- **Ownership percentage:** How much each owns
> **BOI (Beneficial Ownership Information):** Since 2024, US companies must report beneficial owners to FinCEN. Stripe may request this documentation.

## 4. Business Types and Specific Requirements

### Low-Risk Businesses (Fast Approval)

**Examples:** E-commerce, SaaS, consulting, online education, design, development

**Required documentation:**
- Identity document
- Proof of address
- Complete website
- Bank account

**Estimated timeline:** 24-48 hours

### Medium-Risk Businesses (Additional Review)

**Examples:** Travel, health and wellness, regulated products, financial services (consulting)

**Additional documentation:**
- Everything from low risk +
- Specific licenses (if applicable)
- Transaction history (if already operating)
- Supplier information

**Estimated timeline:** 3-7 days

### High-Risk Businesses (Manual Review)

**Examples:** Supplements, CBD (permitted countries), events, donations, digital content

**Additional documentation:**
- Everything from medium risk +
- Specific certifications
- Regulatory compliance proof
- Stripe team interview (possible)
- Previous processing history

**Estimated timeline:** 1-4 weeks

> **Note:** If you operate in a high-risk sector, consider specialized platforms instead of Stripe.

## 5. Identity Verification (Stripe Identity)

### Verification Methods in 2026

- **Static selfie:** Photo of face + document -- 2-5 minutes
- **Video selfie:** Short video saying your name -- 3-5 minutes
- **Live document:** Real-time photo of document -- 1-2 minutes
- **Biometrics:** Facial recognition -- Instant
### Tips to Pass Verification

1. **Proper lighting** — natural light or well-lit environment
2. **Neutral background** — plain wall, no distractions
3. **Visible document** — no glare, corners showing
4. **Uncovered face** — no sunglasses, hat, or mask
5. **Stability** — don't move the phone during capture

### Common Verification Errors

- **Illegible document:** Blurry photo or glare -- Use scanner or good lighting
- **Cut-off document:** Corners not showing -- Frame the entire document
- **Face not recognized:** Significant physical change -- Update document first
- **Expired document:** Validity date passed -- Renew before verifying
- **Low-light selfie:** Dark environment -- Turn on lights or use flash
## 6. Bank Account

### Bank Account Requirements

- **Ownership:** Same name as Stripe account (person or business)
- **Country:** Same country where Stripe account is registered
- **Type:** Checking account (not savings in some countries)
- **Currency:** Local currency of the Stripe account country
- **Status:** Active and in good standing
### Accepted Accounts

- **Personal account:** ✅ Yes (for individuals) -- As long as the name matches
- **Business account:** ✅ Yes (for companies) -- Company name must match
- **Digital bank account:** ✅ Yes (Nubank, Mercury, etc.) -- Digital banks are accepted
- **Savings account:** ❌ No (in most countries) -- Use checking account
- **Joint account:** ✅ Yes -- Account must be in the holder's name
### Bank Account Linking

- **Micro-deposits:** Stripe deposits 2 random amounts -- 1-2 business days
- **Bank login:** Instant verification via API -- Instant
- **Bank statement:** Manual statement upload -- 1-2 business days
## 7. Common Rejection Reasons

### Reason 1: Illegible or Invalid Document

**Problem:** Document photos are blurry, cut off, or have glare.

**Solution:** Use a scanner or take clear photos. Check that the document is not expired.

### Reason 2: Inconsistent Information

**Problem:** Name, address, or date of birth differs between documents.

**Solution:** Verify that ALL information is identical across all submitted documents.

### Reason 3: Incomplete or Inadequate Website

**Problem:** Site under construction, no privacy policy, no contact information.

**Solution:** Complete the website with all required pages before registering.

### Reason 4: Unsupported Business

**Problem:** Your business type is on the prohibited or restricted list.

**Solution:** Check Stripe's prohibited business list before creating an account.

### Reason 5: Unsupported Location

**Problem:** Your country is not on Stripe's supported countries list.

**Solution:** Consider incorporating in a supported country (US, Mexico, Brazil).

### Rejection Reasons Table

- **Illegible document:** 30% -- Use scanner, good lighting
- **Inconsistent information:** 25% -- Verify names and addresses
- **Incomplete website:** 20% -- Have complete site with policies
- **Unsupported business:** 15% -- Check business list
- **Unsupported location:** 10% -- Consider US LLC
## 8. How to Speed Up Approval

### Pre-Registration Checklist

- [ ] Valid identity document with photo
- [ ] Proof of address (less than 3 months old)
- [ ] EIN (if US company)
- [ ] Complete website with policies
- [ ] Bank account in the same country
- [ ] Consistent information across all documents

### Expert Tips

1. **Use the same name on all documents** — ID, passport, bank account, registration
2. **Verify the address** — proof of address must show exactly the same address you enter
3. **Prepare the site first** — don't create an account with an "under construction" site
4. **Choose the correct business type** — the category influences risk analysis
5. **Be patient** — don't create multiple accounts if the first is rejected

### What NOT to Do

- ❌ Don't create multiple accounts — may be considered fraud
- ❌ Don't lie about your business or location — Stripe verifies
- ❌ Don't use expired documents
- ❌ Don't try to verify with someone else's selfie
- ❌ Don't ignore Stripe's emails during the process

## 9. What to Do If Your Account Is Rejected

### Step 1: Read the Rejection Reason

Stripe sends an email explaining the reason. Read it carefully.

### Step 2: Fix the Problem

- **Illegible document:** Send clear photos with scanner
- **Inconsistent information:** Correct the data in registration
- **Incomplete website:** Complete the site and request re-review
- **Unsupported business:** Consider another platform
- **Unsupported location:** Create a US LLC
### Step 3: Request Re-Review

- Access the Stripe dashboard
- Click "Appeal" or "Contact Support"
- Clearly explain what you fixed
- Attach updated documentation

### Step 4: Alternatives

If Stripe definitively rejects:

- **PayPal** — available in 200+ countries
- **Mercado Pago** — Latin America
- **Square** — USA, Canada, Europe
- **Payoneer** — payment accounts

## 10. Documents by Country

### Brazil

- **Identity:** RG, CNH or Passport -- Valid, with photo
- **Residence:** Utility bill or bank statement -- Less than 3 months
- **Company:** CNPJ, Articles of Incorporation -- When applicable
### Mexico

- **Identity:** INE, Passport or Professional License -- Valid
- **Residence:** Utility bill or account statement -- Less than 3 months
- **Company:** RFC, Acta Constitutiva -- When applicable
### USA (LLC)

- **Identity:** Passport or Driver's License -- Valid
- **Residence:** Utility bill or bank statement -- Less than 3 months
- **Company:** EIN Confirmation Letter, Articles of Organization, BOI -- Mandatory
### Europe

- **Identity:** Passport or national ID card -- Valid
- **Residence:** Utility bill or bank statement -- Less than 3 months
- **Company:** Certificate of Incorporation, VAT Number -- When applicable
## Final Approval Checklist

### Before You Start

- [ ] Check if your country is on Stripe's supported list
- [ ] If not, decide: US LLC or alternative platform
- [ ] Prepare all scanned documents
- [ ] Have a complete website with policies

### During Registration

- [ ] Fill in data exactly as on documents
- [ ] Use the same name, address, and date of birth
- [ ] Choose the correct business category
- [ ] Complete identity verification (selfie/video)

### After Registration

- [ ] Link bank account
- [ ] Verify confirmation email
- [ ] Configure 3D Secure
- [ ] Test transactions in test mode
- [ ] Wait for review (24-72 hours)

## Conclusion

**Stripe needs correct documents, consistent information, and a professional website** to approve your account. Most rejections happen from avoidable errors: illegible documents, inconsistent data, or incomplete websites.

Prepare everything in advance, follow the checklist, and your account will be approved quickly. If you need help with structuring, count on **Sotomayor Consulting International** for advisory on US company formation and Stripe account setup.
